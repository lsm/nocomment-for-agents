...
func TestGoFilesListsTrackedSources(t *testing.T) {
	...
	for _, path := range files {
		if path == "cmd/nocomment/main.go" {
			return
		}
	}
	t.Fatalf("goFiles() = %d files, missing cmd/nocomment/main.go", len(files))
}
...
func TestScanIgnoresWhatStripWillNotRemove(t *testing.T) {
	for name, src := range map[string]string{
		"example output":     "package p\n\nfunc ExampleF() {\n\tf()\n\t// Output:\n\t// one\n\t// two\n}\n",
		"unordered output":   "package p\n\nfunc ExampleG() {\n\tg()\n\t// Unordered output:\n\t// a\n\t// b\n}\n",
		"generated header":   "// Code generated by thing. DO NOT EDIT.\n\npackage p\n\nvar X = 1\n",
		"cgo preamble":       "package p\n\n/*\n#include <stdlib.h>\n*/\nimport \"C\"\n",
		"cgo parenthesized":  "package p\n\nimport (\n\t/*\n\t\t#include <stdio.h>\n\t*/\n\t\"C\"\n)\n",
		"output after prose": "package p\n\nfunc ExampleH() {\n\th()\n\t// Output:\n\t// only this block survives\n}\n",
		"canonical import":   "package p // import \"example.com/p\"\n\nvar X = 1\n",
	} {
		t.Run(name, func(t *testing.T) {
			if got := scan([]byte(src)); len(got) != 0 {
				t.Fatalf("scan() = %d spans, want 0: %q", len(got), src[got[0].start:got[0].end])
			}
			out, err := strip([]byte(src))
			if err != nil {
				t.Fatal(err)
			}
			if before, after := len(comments([]byte(src))), len(comments(out)); before != after {
				t.Fatalf("strip() removed %d comment(s) scan() reported none of", before-after)
			}
		})
	}
}

func TestScanCountsEveryCommentStripRemoves(t *testing.T) {
	for name, src := range map[string]string{
		"prose only":            "package p\n\n// gone\nvar X = 1\n",
		"prose beside output":   "package p\n\n// gone\nfunc ExampleF() {\n\tf()\n\t// Output:\n\t// kept\n}\n",
		"prose beside cgo":      "package p\n\n/*\n#include <stdlib.h>\n*/\nimport \"C\"\n\n// gone\nvar X = 1\n",
		"prose beside generate": "package p\n\n// gone\n\n//go:embed x.txt\nvar X string\n",
		"directive lookalikes":  "//go:buildings are tall\n//go:embedding\n\npackage p\n",
	} {
		t.Run(name, func(t *testing.T) {
			out, err := strip([]byte(src))
			if err != nil {
			...
			removed := len(comments([]byte(src))) - len(comments(out))
			if got := len(scan([]byte(src))); got < removed {
				t.Fatalf("scan() = %d spans but strip() removed %d", got, removed)
			}
		})
	}
}

func TestCheckAllowlistClearsAFileHoldingOnlyExampleOutput(t *testing.T) {
	dir := t.TempDir()
	path := writeFile(t, filepath.Join(dir, "example_test.go"), "package p\n\nfunc ExampleF() {\n\tf()\n\t// Output:\n\t// one\n}\n")
	allow := writeFile(t, filepath.Join(dir, "allowlist.txt"), "")
	if code := checkAllowlist([]string{path}, allow); code != 0 {
		t.Fatalf("checkAllowlist() = %d, want 0: an example's output block is not prose a maintainer can delete", code)
	}
}

func TestScanCountsMarkerShapedProseOutOfPosition(t *testing.T) {
	for name, tc := range map[string]struct {
		src  string
		want int
	}{
		"output prose at package scope": {
			src:  "package p\n\n// Output: this is how the hub fans out envelopes, and it is\n// three paragraphs long in the real version.\nfunc F() {}\n",
			want: 2,
		},
		"import prose on a function": {
			src:  "package p\n\n// import the config before calling this, or the allowlist\n// is empty and every source resolves to nothing.\nfunc F() {}\n",
			want: 2,
		},
		"generated header after the package clause": {
			src:  "package p\n\n// Code generated by hand. DO NOT EDIT.\nvar X = 1\n",
			want: 1,
		},
		"output block outside an example": {
			src:  "package p\n\nfunc F() {\n\t// Output:\n\t// one\n}\n",
			want: 2,
		},
	} {
		t.Run(name, func(t *testing.T) {
			if got := len(scan([]byte(tc.src))); got != tc.want {
				t.Fatalf("scan() = %d spans, want %d: a marker out of position is prose", got, tc.want)
			}
		})
	}
}

func TestScanStillExemptsMarkersInPosition(t *testing.T) {
	for name, src := range map[string]string{
		"example output":   "package p\n\nfunc ExampleF() {\n\tf()\n\t// Output:\n\t// one\n}\n",
		"generated header": "// Code generated by hand. DO NOT EDIT.\n\npackage p\n\nvar X = 1\n",
		"canonical import": "package p // import \"example.com/p\"\n\nvar X = 1\n",
	} {
		t.Run(name, func(t *testing.T) {
